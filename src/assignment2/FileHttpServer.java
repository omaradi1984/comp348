 ...
 *
 *<P>
 * An application server that processes incoming http requests for resources.
 *</P>
 *<P>
 ...
 * java filehttpserver.java <<port>> <<number of threads>>
 * Example: java filehttpserver.java 8000 80
 *</P>
 *                                                                              
 *<DL>
 ...
 *<DD> Compile:    javac filehttpserver.java</DD>
 *<DD> Run:        java filehttpserver  <<port>> <<number of threads>></DD>
 *<DD> Document:   javadoc filehttpserver.java</DD>
 *</DL>
 */
/**
*
...
*
*<P>
* public class FileHttpServer {<BR>
...
public class FileHttpServer {
	...
	static class LogWriter {
		...
		private static final Logger accessLogger = Logger
				.getLogger(FileHttpServer.class.getName() + ".accessLogger");
		private static final Logger errorLogger = Logger
				.getLogger(FileHttpServer.class.getName() + ".errorLogger");
	...
	/* Different implementation of logging incoming requests or errors
	 * static class AccessLogger {
	 * 
	 * private static final String LOG_FILE_NAME = "requests"; private static
	 * final SimpleDateFormat dateFormat = new
	 * SimpleDateFormat("dd/MMM/yyyy:HH:mm:ss");
	 * 
	 * public static void log(HttpExchange t, File file, String filePath) { try
	 * (BufferedWriter writer = new BufferedWriter(new FileWriter(LOG_FILE_NAME,
	 * true))) { String timeStamp = dateFormat.format(new Date()); Instant
	 * instant = Instant.now(); String timezone =
	 * ZoneId.systemDefault().getRules().getOffset(instant).toString().replace(
	 * ":", ""); String remoteAddress = t.getRemoteAddress().getHostString();
	 * String method = t.getRequestMethod(); String protocol = t.getProtocol();
	 * int responseCode = t.getResponseCode(); long fileSize = file.length();
	 * writer.write(remoteAddress + " - - " + "[" + timeStamp + " " + timezone
	 * +"] \"" + method + " " + filePath +" " + protocol +"\" " + responseCode +
	 * " " + fileSize); writer.newLine(); } catch (IOException e) {
	 * System.err.println("Error writing to log file: " + e.getMessage()); } } }
	 * 
	 * static class ErrorLogger {
	 * 
	 * private static final String LOG_FILE_NAME = "errors"; private static
	 * final SimpleDateFormat dateFormat = new
	 * SimpleDateFormat("dd/MMM/yyyy:HH:mm:ss");
	 * 
	 * public static void log(HttpExchange t, File file, String filePath) { try
	 ...
	 * instant = Instant.now(); String timezone =
	 * ZoneId.systemDefault().getRules().getOffset(instant).toString().replace(
	 * ":", ""); writer.write(t.getRemoteAddress().getHostString() + " - - " +
	 * "[" + timeStamp+ " " + timezone +"] \"" + t.getRequestMethod()+ " " +
	 * filePath +" " + t.getProtocol()+"\" " + t.getResponseCode() + " " +
	 * file.length()); writer.newLine(); // errorLogger.log(Level.INFO,
	 * t.getRemoteAddress().getHostString() + " - - " + "[" + timeStamp+ " " +
	 * timezone +"] \"" + t.getRequestMethod()+ " " + filePath +" " +
	 * t.getProtocol()+"\" " + t.getResponseCode() + " " + file.length()); }
	 * catch (IOException e) { System.err.println("Error writing to log file: "
	 * + e.getMessage()); } } }
	 */
}